...
class SuiviDemandeAPIView(APIView):
    ...
    def get_stage_info(self, stagiaire, request):
        ...
        try:
            ...
            try:
                if stagiaire.a_ete_renouvele and stagiaire.nouveau_stage:  # ✅ bon champ
                    nouveau_stage = stagiaire.nouveau_stage
                ...
                logger.warning(f"Erreur stage renouvelé: {e}")

            # Remplacer le bloc stages_precedents par :
            try:
                # ✅ Bon champ : stage_precedent
                stages_precedents = Stagiaire.objects.filter(stage_precedent=stagiaire)
                for stage_precedent in stages_precedents:
                    historique_stages.append({
                        ...
                        "est_renouvelable": stage_precedent.statut_actuel == "Terminé" 
                            and not stage_precedent.a_ete_renouvele,
                    })
            except Exception as e:
                logger.warning(f"Erreur historique stages: {e}")

            # Remplacer informations_renouvellement par :
            informations_renouvellement = {
                "a_ete_renouvele": getattr(stagiaire, 'a_ete_renouvele', False),
                "nouveau_stage_id": stagiaire.nouveau_stage.id if stagiaire.nouveau_stage else None,
                "peut_etre_renouvele": stagiaire.statut_actuel == "Terminé" 
                    and not getattr(stagiaire, 'a_ete_renouvele', False),
                # ✅ Bon champ
                "date_renouvellement": stagiaire.date_finalisation_renouvellement.isoformat() 
                    if stagiaire.date_finalisation_renouvellement else None,
            }
...
class ModifierDemandeAPIView(APIView):
    ...
    def serialize_demande(self, demande, request):
        ...
        photo_passeport_url = None
        if demande.photo_passeport and demande.photo_passeport.name:
            photo_passeport_url = request.build_absolute_uri(demande.photo_passeport.url)
        ...
        return {
            ...
            # ✅ Appel correct
            'documents': suivi_view.get_demande_documents(demande, request)
        }
...
class PreAccepterDemandeAPIView(APIView):
    ...
    def generer_pdf_temporaire(self, demande, donnees):
        try:
            # ✅ Ne pas passer numero_convention, save() le génère
            convention = ConventionStage.objects.create(
                est_temporaire=True,
                demande=demande,
                stagiaire=None,
                # ✅ Retiré : numero_convention=f"TEMP-{demande.tracking_id}-{int(time.time())}"
            )
...
class FinaliserAcceptationAPIView(APIView):
    ...
    def creer_stagiaire_final(self, demande, donnees):
        ...
        try:
            ...
            stagiaire_existant = Stagiaire.objects.filter(demande=demande).first()
            if stagiaire_existant:
                logger.info(f"Stagiaire existant trouvé: {stagiaire_existant.id}")
...
class RegenererConventionAPIView(APIView):
    ...
    def regenerer_pdf(self, convention, demande, donnees, signataire_info):
        """Régénère le PDF avec le signataire choisi — réutilise la logique existante"""
        try:
            from reportlab.lib.pagesizes import A4
            from reportlab.lib.units import cm
            from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer
            ...
            # Helpers réutilisés depuis PreAccepterDemandeAPIView
            pre_view = PreAccepterDemandeAPIView()
            duree_mois = pre_view.calculer_duree_mois_temporaire(donnees["date_debut"], donnees["date_fin"])
            date_debut_format = pre_view.date_format_francais(donnees["date_debut"])
            date_fin_format = pre_view.date_format_francais(donnees["date_fin"])
            ...
            nom_fichier = f"convention_{demande.tracking_id}_{int(time.time())}.pdf"
            convention.fichier.save(nom_fichier, ContentFile(pdf_content), save=True)

            return True

        except Exception as e:
            logger.error(f"Erreur régénération PDF: {str(e)}", exc_info=True)
            return False
...
class DemandesAttestationBaseAPI(APIView):
    permission_classes = [IsAuthenticated]
    statut_filter = None  # Surcharger dans les sous-classes
    ...
    def get(self, request):
        try:
            ...
            queryset = DemandeAttestation.objects.select_related(
                'stagiaire', 'stagiaire__etablissement'
            )

            # ✅ Appliquer le filtre de statut de la classe
            ...
            demandes_data = [
                self.serialize_demande(d, request)
            ...
            stats = {
                ...
                'traitees': DemandeAttestation.objects.filter(statut='traitee').count(),
            }
            ...
            logger.error(f"Erreur récupération demandes attestation: {e}")
    ...
    def serialize_demande(self, demande, request):
        ...
        fichiers = {
            'rapport_stage': request.build_absolute_uri(demande.rapport_stage.url)
                if demande.rapport_stage and demande.rapport_stage.name else None,
            'demande_manuscrite': request.build_absolute_uri(demande.demande_manuscrite.url)
                if demande.demande_manuscrite and demande.demande_manuscrite.name else None,
        }
        # ✅ Ajouter attestations si elles existent
        if hasattr(demande, 'attestation_generee') and demande.attestation_generee \
                and demande.attestation_generee.name:
            fichiers['attestation_generee'] = request.build_absolute_uri(
                demande.attestation_generee.url)
        if hasattr(demande, 'attestation_signee') and demande.attestation_signee \
                and demande.attestation_signee.name:
            fichiers['attestation_signee'] = request.build_absolute_uri(
                demande.attestation_signee.url)
        ...
        return {
            ...
            'stagiaire': {
                ...
                'date_debut': demande.stagiaire.date_debut.isoformat()
                    if demande.stagiaire.date_debut else None,
                'date_fin': demande.stagiaire.date_fin.isoformat()
                    if demande.stagiaire.date_fin else None,
                ...
                'etablissement': {
                    'nom': demande.stagiaire.etablissement.nom,
                    'email': demande.stagiaire.etablissement.email,
                } if demande.stagiaire.etablissement else None,
            }
        }

# ✅ Les 5 classes deviennent de simples sous-classes
class DemandesAttestationAPI(DemandesAttestationBaseAPI):
    statut_filter = None  # Toutes

class DemandesAttestationEnAttenteAPI(DemandesAttestationBaseAPI):
    statut_filter = 'en_attente'

class DemandesAttestationRefuseAPI(DemandesAttestationBaseAPI):
    statut_filter = 'refusee'

class DemandesAttestationApprouveAPI(DemandesAttestationBaseAPI):
    statut_filter = 'approuvee'

class DemandesAttestationTraiteeAPI(DemandesAttestationBaseAPI):
    statut_filter = 'traitee' 
    ...
    def post(self, request, tracking_id=None):
        ...
        try:
            ...
            if existing_demande and is_new_attempt:
                ...
                if existing_demande.rapport_stage and existing_demande.rapport_stage.name:
                    try:
                        existing_demande.rapport_stage.delete(save=False)  # ✅ Via storage Django
                    except Exception as e:
                        logger.warning(f"Impossible de supprimer l'ancien rapport: {e}")

                if existing_demande.demande_manuscrite and existing_demande.demande_manuscrite.name:
                    try:
                        existing_demande.demande_manuscrite.delete(save=False)  # ✅ Via storage Django
                    except Exception as e:
                        logger.warning(f"Impossible de supprimer l'ancienne demande: {e}")
...
class ApprouverDemandeAttestationAPI(APIView):
    ...
    def post(self, request, demande_id):
        try:
            with transaction.atomic():
                ...
                # 📄 GÉNÉRER ET SAUVEGARDER L'ATTESTATION PDF DANS LE CHAMP APPROPRIÉ
                try:
                    attestation_pdf = self.generer_attestation_pdf(demande_attestation)
                    ...
                    logger.info(f"✅ Attestation PDF générée et sauvegardée dans attestation_generee pour {demande_attestation.stagiaire}")
    ...
    def generer_attestation_pdf(self, demande_attestation):
        """Génère le PDF de l'attestation de stage"""
        buffer = BytesIO()
        ...
        p.drawRightString(width - 2*cm, text_y_position, "Le Directeur Général")
        p.drawRightString(width - 2*cm, 3*cm, "Karimou CHABI SIKA")
        
        # Pied de page avec informations
        p.setFont("Helvetica", 9)
        p.setFillColorRGB(0.4, 0.4, 0.4)  # Gris foncé
    ...
    def delete(self, request, demande_id):
        try:
            with transaction.atomic():
                ...
                # ✅ Vérifier .name avant delete
                for champ in ['rapport_stage', 'demande_manuscrite', 
                            'attestation_generee', 'attestation_signee']:
                    fichier = getattr(demande_attestation, champ, None)
                    if fichier and fichier.name:
                        try:
                            fichier.delete(save=False)
                        except Exception as e:
                            logger.warning(f"Erreur suppression {champ}: {e}")
            ...
            return Response({'success': False, 'message': 'Non trouvée'}, status=404)
        except Exception as e:
            logger.error(f"Erreur suppression attestation {demande_id}: {e}")
            return Response({'success': False, 'message': str(e)}, status=500)
    ...
    def post(self, request, demande_id):
        try:
            with transaction.atomic():
                ...
                if demande_attestation.attestation_signee and demande_attestation.attestation_signee.name:
                    try:
                        demande_attestation.attestation_signee.delete(save=False)  # ✅ Via storage Django
                    except Exception as e:
                        logger.warning(f"Impossible de supprimer l'ancienne attestation: {e}")
    ...
    def get(self, request, demande_id):
        try:
            ...
            if not demande_attestation.attestation_signee \
                    or not demande_attestation.attestation_signee.name:
                return Response({
                    'success': False,
                    'message': "L'attestation signée n'est pas disponible."
                }, status=404)

            # ✅ Utiliser le storage Django (compatible local ET S3)
            if not demande_attestation.attestation_signee.storage.exists(
                demande_attestation.attestation_signee.name
            ):
                return Response({
                    'success': False,
            ...
            # ✅ FileResponse avec open() via storage Django
            file_handle = demande_attestation.attestation_signee.open('rb')
            filename = os.path.basename(demande_attestation.attestation_signee.name)
            response = FileResponse(file_handle, content_type='application/pdf')
            response['Content-Disposition'] = f'attachment; filename="{filename}"'
            return response

        except DemandeAttestation.DoesNotExist:
            return Response({'success': False, 'message': 'Demande non trouvée'}, status=404)
        except Exception as e:
            logger.error(f"Erreur téléchargement attestation signée {demande_id}: {e}")
            return Response({'success': False, 'message': str(e)}, status=500)