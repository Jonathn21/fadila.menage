...
const PendingInternships: React.FC = () => {
  ...
  const fetchInternships = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get("/demande-en-acceptation/", {
        params: {
          q: searchQuery,
    ...
    const tableHtml = `
      ...
      <body>
        ...
        <div style="margin-top: 20px; color: #666;">
          Total: ${filteredInternships.length} demande(s) en acceptation
        </div>
      </body>
      </html>
    `;
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-2">
            <div>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight">Demandes en acceptation</h1>
              <CardDescription className="text-muted-foreground">
                Gérez et suivez l'ensemble des demandes de stage en acceptation
              </CardDescription>
            </div>
          </div>
        </div>
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2 sm:gap-0">
                  <h3 className="font-semibold">
                    {filteredInternships.length} demande{filteredInternships.length !== 1 ? 's' : ''} en acceptation
                  </h3>
                </div>
                          ...
                          <tr>
                            <td colSpan={columns.length} className="p-4 sm:p-6 md:p-8 text-center">
                              <div className="text-center py-12">
                                ...
                                <h3 className="font-medium text-lg mb-1">Aucune demande en acceptation</h3>
