...
const PendingInternships: React.FC = () => {
  ...
  const fetchInternships = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get("/demande-refusees/", {
        params: {
          q: searchQuery,
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger les demandes refusées.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const printTable = () => {
    ...
    const tableHtml = `
      ...
      <html>
      <head>
        <title>Demandes Refusées - ${new Date().toLocaleDateString()}</title>
        <style>
          ...
          .note { margin-top: 20px; padding: 10px; background-color: #fef3c7; border-radius: 4px; font-size: 12px; }
        </style>
      </head>
      <body>
        <div class="header">
          <div>
            ...
            <div class="subtitle">Historique des demandes de stage refusées</div>
          </div>
          <div>Lomé le ${new Date().toLocaleDateString()}</div>
        </div>
        <div class="info">
          <strong>Total: ${filteredInternships.length} demande(s) refusée(s)</strong>
        </div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            ${filteredInternships.map(internship => {
              ...
              const daysSinceSubmission = Math.floor((today.getTime() - submissionDate.getTime()) / (1000 * 3600 * 24));
              ...
              return `
                <tr>
                  ...
                  <td><span class="status-badge">Refusée</span></td>
                  <td>${submissionDate.toLocaleDateString()}</td>
                  <td>${daysSinceSubmission} jour(s)</td>
                </tr>
              `;
            }).join('')}
          </tbody>
        ...
        <div class="note">
          <strong>Note:</strong> Cette liste représente l'historique des demandes refusées. 
          Les données peuvent être utilisées pour l'analyse des motifs de refus.
        </div>
      </body>
      </html>
    `;
  ...
  const calculateDaysSinceSubmission = (dateSoumission: string) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: string, item: Internship) => {
        ...
        const daysSinceSubmission = calculateDaysSinceSubmission(value);
              ...
              <>
                ...
                <div className="md:hidden space-y-4">
                  {filteredInternships.length === 0 ? (
                    ...
                    filteredInternships.map((internship) => {
                      const daysSinceSubmission = calculateDaysSinceSubmission(internship.date_soumission);
                          ...
                          <CardContent className="p-4">
                            <div className="space-y-4">
                              ...
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0">
                                ...
                                <Badge variant="destructive" className="capitalize shrink-0 ml-2">
                                  Refusée
                                </Badge>
                              </div>
