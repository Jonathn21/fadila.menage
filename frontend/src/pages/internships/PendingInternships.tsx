...
const PendingInternships: React.FC = () => {
  ...
  const fetchInternships = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get("/demande-en-attente/", {
        params: {
          q: searchQuery,
  ...
  const printTable = () => {
    ...
    const tableHtml = `
      ...
      <html>
      <head>
        <title>Demandes en Attente - ${new Date().toLocaleDateString()}</title>
        <style>
      ...
      <body>
        ...
        <div class="info">
          <strong>Total: ${filteredInternships.length} demande(s) en attente</strong>
        </div>
        <table>
          <thead>
            <tr>
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      render: (value: string, item: Internship) => {
        ...
        const daysWaiting = calculateDaysWaiting(value);
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-2">
            <div>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight">Demandes en attente</h1>
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2 sm:gap-0">
                  <h3 className="font-semibold">
                    {filteredInternships.length} demande{filteredInternships.length !== 1 ? 's' : ''} en attente
                  </h3>
                </div>
                ...
                <div className="md:hidden space-y-4">
                  {filteredInternships.length === 0 ? (
                    ...
                    filteredInternships.map((internship) => {
                      const daysWaiting = calculateDaysWaiting(internship.date_soumission);
                              ...
                              <div className={`p-3 rounded-lg text-sm ${
                                ...
                                {daysWaiting > 7 && (
                                  <p className="text-xs mt-1">
                                    Cette demande nécessite une attention prioritaire
                                  </p>
                                )}
                              </div>
