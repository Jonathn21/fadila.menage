...
const PendingInternships: React.FC = () => {
  ...
  const [sortConfig, setSortConfig] = useState<SortConfig>({ field: "date_soumission", direction: "desc" });
  ...
  const fetchInternships = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get("/demande-en-traitement/", {
        params: {
          q: searchQuery,
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger les demandes en cours de traitement.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const printTable = () => {
    ...
    const tableHtml = `
      ...
      <html>
      <head>
        <title>Demandes en Cours de Traitement - ${new Date().toLocaleDateString()}</title>
        <style>
      ...
      <body>
        <div class="header">
          <div>
            ...
            <div class="subtitle">Demandes de stage en cours d'analyse et de validation</div>
          </div>
          <div>Lomé le ${new Date().toLocaleDateString()}</div>
        </div>
        <div class="info">
          <strong>Total: ${filteredInternships.length} demande(s) en cours de traitement</strong>
        </div>
        <table>
          <thead>
            <tr>
              ...
              <th>Jours en traitement</th>
            </tr>
          </thead>
          <tbody>
            ${filteredInternships.map(internship => {
              const submissionDate = new Date(internship.date_soumission);
              const today = new Date();
              const daysInProcess = Math.floor((today.getTime() - submissionDate.getTime()) / (1000 * 3600 * 24));
              const urgencyClass = daysInProcess > 30 ? "urgency-high" : daysInProcess > 15 ? "urgency-medium" : "";
        ...
        <div style="margin-top: 20px; font-size: 12px; color: #999;">
          ...
          <span style="background-color: #fee2e2; padding: 2px 6px; margin: 0 5px;">+30 jours</span> - 
          <span style="background-color: #fef3c7; padding: 2px 6px; margin: 0 5px;">15-30 jours</span> - 
          <span style="padding: 2px 6px; margin: 0 5px;">Moins de 15 jours</span>
        </div>
      </body>
      </html>
    `;
  ...
  const handleResetFilters = () => {
    ...
    setSortConfig({ field: "date_soumission", direction: "desc" });
  };
  ...
  // Calculer les jours en traitement
  ...
  const getProcessBadgeVariant = (daysInProcess: number) => {
    if (daysInProcess > 30) return "destructive";
    if (daysInProcess > 15) return "secondary";
    return "default";
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value: string, item: Internship) => {
        ...
        const daysInProcess = calculateDaysInProcess(value);
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-2">
            <div>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold tracking-tight">Demandes en cours de traitement</h1>
              <CardDescription className="text-muted-foreground">
                Gérez et suivez l'ensemble des demandes de stage en cours de traitement
              </CardDescription>
            </div>
          </div>
        </div>
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2 sm:gap-0">
                  <h3 className="font-semibold">
                    {filteredInternships.length} demande{filteredInternships.length !== 1 ? 's' : ''} en cours de traitement
                  </h3>
                </div>
                          ...
                          <tr>
                            <td colSpan={columns.length} className="p-4 sm:p-6 md:p-8 text-center">
                              <div className="text-center py-12">
                                ...
                                <h3 className="font-medium text-lg mb-1">Aucune demande en cours de traitement</h3>
                ...
                <div className="md:hidden space-y-4">
                  {filteredInternships.length === 0 ? (
                    ...
                    filteredInternships.map((internship) => {
                      const daysInProcess = calculateDaysInProcess(internship.date_soumission);
                          ...
                          <CardContent className="p-4">
                            <div className="space-y-4">
                              ...
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0">
                                ...
                                <Badge 
                                  variant={getProcessBadgeVariant(daysInProcess)} 
                              ...
                              {/* Indicateur de délai de traitement */}
                              <div className={`p-3 rounded-lg text-sm ${
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0">
                                  <span className="font-medium">
                                    {daysInProcess > 30 ? 'Retard critique' : 
                                ...
                                {daysInProcess > 30 && (
                                  <p className="text-xs mt-1">
                                    Cette demande dépasse le délai de traitement recommandé
                                  </p>
                                )}
                                {daysInProcess > 15 && daysInProcess <= 30 && (
                                  <p className="text-xs mt-1">
                                    Cette demande approche du délai maximum recommandé
                                  </p>
                                )}
                              </div>
